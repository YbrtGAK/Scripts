...
#show: course.with(
  title: [NdC : Equations de conservation des écoulements monophasiques],
...
- Condition de validité :
  - La longueur caractéristique du système $L_c >> "libre parcours moyen" $ *$l$*. Pour s'en assurer, on introduit le nombre adimensionnel de Knudsen : *$K_n=l/L$*. Ainsi l'hypothèse du milieu continu est valide si $K_n << 1.$ 
...
- La connaissance de l'accélération du fluide :
  - Considérons une grandeur physique scalaire ou vectorielle quelconque du fluide, appelée G. Par définition, $G accent(=,"^") G(vec(x),t)=G(x,y,z,t)$. On donne la différentielle totale de G : 
   ...
   $ "dG" &= (diff G)/(diff x)."dx" + (diff G)/(diff y)."dy" + (diff G)/(diff z)."dz" + (diff G)/(diff t)."dt" \
   arrow.double "dG"/"dt" &= (diff G)/(diff x)."dx"/"dt" + (diff G)/(diff y)."dy"/"dt" + (diff G)/(diff z)."dz"/"dt" + (diff G)/(diff t) \
   arrow.double "dG"/"dt" &= (diff G)/(diff x)."vx" + (diff G)/(diff y)."vy" + (diff G)/(diff z)."vz" + (diff G)/(diff t) $
...
L'objectif de cette partie est de définir le débit masse. Soit un fluide s'écoulant dans un tube de section S. Considérons une surface infinitésimale dS de centre M. On appelle dm la masse s'écoulant au travers de cette section pendant un temps dt : 
$ &(d^2m)/"dt.dS" = rho(vec(x),t).vec(v)(vec(x),t).vec(n) \
arrow.double &d m = integral_(M in S)((rho(vec(x),t).vec(v)(vec(x),t).vec(n))."dS")."dt" $ 
...
$ "dm"/"dt" = - integral.cont_(M in S) rho(vec(x),t).vec(v).d vec(S)_"ext" $
...
rho. derpt(vec(u),t) + rho. vec(u). nabla vec(u) &=rho. vec(g) - nablav(-P(vec(x),t).I + tensor(sigma_D)) \

so rho. derpt(vec(u),t) + rho. vec(u). nabla vec(u) &=rho. vec(g) - nablav(-P.I) + nablav tensor(sigma_D) \

so rho. derpt(vec(u),t) + rho. vec(u). nabla vec(u) &=rho. vec(g) -P. nablav I + I. nablav(-P) + nablav tensor(sigma_D) \
...
supplement : "l'équation",
block : true)[$
rho. derpt(vec(u),t) + rho. vec(u). nabla vec(u) &=rho. vec(g) - nablav P + nablav tensor(sigma_D) 
$] <eq_cons_QDM>
...
rho. derpt(vec(u),t) + rho. vec(u). nabla vec(u) &=rho. vec(g) - nablav P + nablav tensor(sigma_D) 
$
En multipliant chaque terme par la vitesse $vec(u)$, on obtient le bilan des puissances volumiques  : 
$
&vec(u). rho. derpt(vec(u),t) + vec(u). rho. vec(u). nabla vec(u) = vec(u). rho. vec(g) - vec(u). nablav P + vec(u).nablav tensor(sigma_D) \

so & 1/2. rho. derpt(vec(u)^2,t) + 1/2.vec(u). rho. nabla vec(u)^2 = vec(u). rho. vec(g) - vec(u). nablav P + vec(u).nablav tensor(sigma_D) \
$
Finalement, on obtient la conservation de l'énergie mécanique : 
#math.equation(numbering:"(1)",
supplement : "l'équation",
block : true)[$
& rho. Der("")(1/2. abs(abs(vec(u)))^2) = -vec(u). nabla P + vec(u). (rho. vec(g)) + vec(u).nablav tensor(sigma_D)
$] <eq_cons_Emeca>
